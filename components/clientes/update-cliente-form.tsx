...
import { updateCliente } from '@/server/cliente-queries';
...
export const UpdateClienteForm = ({ closeDialog }: Props) => {
  const [loading, setLoading] = useState(false);
  const clienteToUpdate = useClienteStore((state) => state.clienteToUpdate);
  const updateClienteFromStore = useClienteStore(
    (state) => state.updateClienteFromStore
  );

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      ci: clienteToUpdate?.ci || '',
      nombre: clienteToUpdate?.nombre || '',
      celular: clienteToUpdate?.celular || '',
      direccion: clienteToUpdate?.direccion || '',
      notasEntrega: clienteToUpdate?.notasEntrega || '',
      razonSocial: clienteToUpdate?.razonSocial || '',
      nit: clienteToUpdate?.nit || '',
    },
  });

  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    if (!clienteToUpdate) return;
    ...
    try {
      ...
      await updateCliente(clienteToUpdate.ci, clienteData);
      updateClienteFromStore({
        ...clienteData,
        puntos: clienteToUpdate.puntos ?? 0,
      });
  ...
  if (!clienteToUpdate) return null;
  ...
  return (
    <Form {...form}>
      <DialogHeader>
        ...
        <DialogDescription>
          Actualiza la información del cliente {clienteToUpdate.nombre}
        </DialogDescription>
      </DialogHeader>
