...
import { addCombo } from '@/server/queries';
...
export const AddComboForm = ({ closeDialog }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast.success('Operacion exitosa!', {
        description: 'El combo se ha agregado correctamente!',
      });
      closeDialog();
    } catch (error) {
      setLoading(false);
  ...
  return (
    <Form {...form}>
      <DialogHeader>
        ...
        <DialogDescription>
          Llena la informacion del combo que deseas agregar
        </DialogDescription>
      </DialogHeader>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <DialogFooter>
          ...
          <Button type="submit" disabled={loading}>
            {loading ? 'Agregando...' : 'Agregar combo'}
          </Button>
        </DialogFooter>
      </form>
    </Form>
